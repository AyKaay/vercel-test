...
function homepage() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/auctions")
      ...
      .then((data) => {
        // Limit to 6 items
        const limitedItems = data.slice(0, 8);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col align-center items-center justify-center">
        ...
        <p className="text-lg font-medium text-center mx-60 mt-8 opacity-90">Explore a world of exciting auctions, where you can find amazing deals on various items and objects. Join us and start bidding on your favorite items today!</p>
          ...
          {auctions.length > 0 ? (
            auctions.map((auction) => (
              <ItemCard
                ...
                src={"https://bg3.wiki/w/images/f/ff/Blazer_of_Benevolence_Icon.png"} 
                name={auction.title}         
                price={auction.startingPrice} 
              />
            ))
          ) : (
            <p className="flex align-center items-center justify-center">No auctions available at the moment.</p>
          )}
        </div>
      </div>
      .{/* Search By Category */}
