...
function HeaderItem({ link, text }) {
  return (
    ...
    >
      <Button radius="sm" variant='light' className="py-6 text-slate-200 dark:text-slate-100">
        <span className="text-slate-900 dark:text-slate-100 font-black">{text}</span>
      </Button>
    </Link>
  );
}
...
function Layout() {
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await axios.delete(`${API_URL}/logged/${id}`);
          ...
          <div className={`font-medium ${containerClasses} ${headerClasses}`}>
            ...
            {logged ? (
              <div className="flex max-md:flex-col gap-5">
                <div className="hover:bg-slate-300 max-md:pb-2 cursor-pointer md:px-3">
                  <CarbonFavorite
                    className="text-3xl max-md:ml-7 md:mt-3 max-md:mt-6"
                    onClick={() => navigate("/watchlist")}
                  />
                </div>
                <div
                  className="md:py-[0.93rem] max-md:pb-4 cursor-pointer hover:bg-slate-300 max-md:py-4"
                  onClick={() => handleLogout()}
                >
                  <HeaderItem link="/dashboard" text="PROFILE" />
                <HeaderItem text="LOGOUT" />
                </div>
              </div>
            ) : (
