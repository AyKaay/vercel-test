...
import { API_URL, formatNumberWithDots, formatRupiah } from "../lib";
...
const AddWatchlist = ({ watch }) => {
  ...
  const getWatchItem = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/watchlist?userId=${watch.userId}&id=${watch.id}`
      );
  ...
  const addWatch = async (data) => {
    try {
      await axios.post(`${API_URL}/watchlist`, data);
      getWatchItem();
    } catch (error) {
      console.log(error);
    }
...
const Product = () => {
  ...
  const getProduct = async () => {
    try {
      const response = await axios.get(`${API_URL}/auctions/${auctId}`);
  ...
  const closeBid = async () => {
    ...
    try {
      const response = await axios.put(`${API_URL}/auctions/${auctId}`, close);
      getProduct();
      console.log("response", response);
    } catch (error) {
      console.log(error);
  ...
  const updateBid = async () => {
    ...
    try {
      const response = await axios.put(`${API_URL}/auctions/${auctId}`, update);
      getProduct();
      console.log("response", response);
    } catch (error) {
      console.log(error);
  ...
  const handleDeleteAuction = async () => {
    ...
    if (cnfrm) {
      try {
        await axios.delete(`${API_URL}/auctions/${auctId}`);
    ...
    <div className="flex flex-col">
      ...
      <section className="flex mb-1 max-[1192px]:flex-col lg:flex-wrap gap-3 justify-center min-[1193px]:justify-between ">
        ...
        <div className="flex  max-[729px]:flex-wrap gap-2 max-[774px]:flex-col max-[774px]:items-center grow">
          ...
          {logged && thereisUser ? (
            //logged component via-red-100 to-yellow-100
            <div className="p-2 flex flex-col justify-between w-full mb-3 bg-gradient-to-r from-transparent  rounded">
              ...
              {/* bid box */}
              <div className="mb-3 bg-gradient-to-r from-transparent via-red-100 to-yellow-100 rounded">
            ...
            //unlogged component
            <div className="p-2 flex flex-col justify-between w-full bg-gradient-to-r from-transparent via-red-100 to-yellow-100 rounded ">
              <title className="prose dark:prose-invert max-w-full max-[774px]:mb-3 flex justify-between ">
                <h2>{product.title}</h2>
                <div
